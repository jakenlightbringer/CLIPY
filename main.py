...
def enemyLogic(enemy, venom, enweb, player, attackTurn):
    if(enemy.enemySP > 1):
        abilitySelection = random.randrange(1, 3)
        print(abilitySelection)
    else:
        abilitySelection = 1
    if(abilitySelection == 1):
        player.baseHealth -= enemy.attackValue
        player.baseHealth += player.baseDefense
        playerCurrentHP = player.baseHealth
        enemy.enemySP -= 2
        #print(playerCurrentHP)
        return abilitySelection
    elif(abilitySelection == 2):
        venomAttackValue = enemy.attackValue + venom.spellEffect
        player.baseHealth -= venomAttackValue
        playerCurrentHP = player.baseHealth
        #print(playerCurrentHP)
        return abilitySelection
    elif(abilitySelection == 3):
        attackTurn = False
        enemy.baseSP -= 3
        #print(playerCurrentHP)
        return attackTurn
...
def main():
    playerName = startPlayer.getName()
